    ...
    def enable(self):
        if not self.enable_status:
            if self.diagnostic_code == 'A012':
                ...
                logger.log(
                    f'Enable signal not send due to diagnostic code {self.diagnostic_code}')
                self.soft_drive_message = (
                    f'Enable signal not send due to diagnostic code {self.diagnostic_code}')
        else:
            logger.log(
                'Enable signal not send due to: enable signal aready present.')
            self.soft_drive_message = (
                'Enable signal not send due to: enable signal aready present.')
    ...
    def release_halt(self):
        if self.enable:
            if not self.halt_status:
                ...
                logger.log(
                    'Drive halt signal not send due to: drive halt signal aready present.')
                self.soft_drive_message = (
                    'Drive halt signal not send due to: drive halt signal aready present.')
                return
        else:
            logger.log(
                'Drive halt signal not send due to: enable signal not present.')
            self.soft_drive_message = (
                'Drive halt signal not send due to: enable signal not present.')
            return
        ...
        if self.enable_status:
            if self.halt_status:
                if self.diagnostic_code == 'A211':
                    ...
                    logger.log(
                        f'Start signal not sent due to incorrect diagnostic code: {self.diagnostic_code}')
                    self.soft_drive_message = (
                        f'Start signal not sent due to incorrect diagnostic code: {self.diagnostic_code}')
                    return
            else:
                logger.log(
                    'Start signal not sent because halt was not released')
                self.soft_drive_message = (
                    'Start signal not sent because halt was not released')
                return
        else:
            logger.log(
                'Start signal not sent because enable signal is zero.')
            self.soft_drive_message = (
                'Start signal not sent because enable signal is zero.')
            return
