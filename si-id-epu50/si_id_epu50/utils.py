...
def schedule(interval):
    def decorator(func):
        def periodic(scheduler, interval, action, actionargs=(), kwargs={}):
            scheduler.enter(
                interval, 1, periodic, (scheduler, interval, action, actionargs, kwargs)
            )
...
def bsmp_send(command_type, variableID=0x00, value=0x00, size=1):
    send_message = (
        [0x00, command_type] + [c for c in struct.pack("!h", size + 1)] + [variableID]
    )
    if size == 1:
...
def get_phase_hal(val):
    bsmp_enable_message = bsmp_send(
        _cte.BSMP_READ, variableID=_cte.HALT_CH_SI, value=val
    ).encode()
...
def get_file_handler(file: str):
    ...
    fhr = logging.handlers.RotatingFileHandler(file, maxBytes=1000000, backupCount=10)
