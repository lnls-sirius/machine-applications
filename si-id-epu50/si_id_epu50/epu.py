...
def set_digital_signal(
    val: bool,
    bsmp_enable_message: bytes,
    drive1: EcoDrive,
    drive2: EcoDrive,
...
class Epu:
    ...
    def __init__(self, args, callback_update=lambda: 1):
        ...
        if not hasattr(self, "initialized"):
            ...
            self.monitor_phase_movement_thread = Thread(
                target=self._monitor_phase_movement, daemon=True
            )
            self.monitor_gap_movement_thread = Thread(
                target=self._monitor_gap_movement, daemon=True
            )
        ...
        for drive, var in zip(drives, variables):
            while True:
                try:
                    setattr(
                        self,
                        f"{var}_target_position",
                        float(drive.get_target_position()),
                    )
                    setattr(
                        self, f"{var}_target_velocity", float(drive.get_max_velocity())
                    )
    ...
    def _monitor_gap_movement(self):
        self._monitor_movement(
            self.gap_start_event, self.a_drive, "gap", "Gap movement"
        )

    def _monitor_phase_movement(self):
        self._monitor_movement(
            self.phase_start_event, self.i_drive, "phase", "Phase movement"
        )
    ...
    def _reconnect_io_serial(self):
        ...
        """
        self.gpio_connected, self.rs485_connected = (
            self._gpio_socket.connected,
            self._serial_socket.connected,
        )
    ...
    def _read_drive(self, drive):
        ...
        with self._epu_lock:
            try:
                ...
                logger.debug(
                    "Drive %s did not respond during standstill monitoring.", drive
                )
                logger.debug(e)
                raise
    ...
    def _read_drive_a(self):
        try:
            (
                self.a_resolver_gap,
                self.a_encoder_gap,
                self.a_target_position,
                self.a_diag_code,
                self.a_target_velocity,
            ) = self._read_drive(self.a_drive)
            self.gap_target_velocity = self.a_target_velocity
        except (ValueError, TypeError):
            pass
    ...
    def _read_drive_b(self):
        try:
            (
                self.b_resolver_gap,
                self.b_encoder_gap,
                self.b_target_position,
                self.b_diag_code,
                self.b_target_velocity,
            ) = self._read_drive(self.b_drive)
        except (ValueError, TypeError):
            pass
    ...
    def _read_drive_i(self):
        try:
            (
                self.i_encoder_phase,
                self.i_resolver_phase,
                self.i_target_position,
                self.i_diag_code,
                self.i_target_velocity,
            ) = self._read_drive(self.i_drive)
            self.phase_target_velocity = self.i_target_velocity
        except (ValueError, TypeError):
            pass
    ...
    def _read_drive_s(self):
        try:
            (
                self.s_resolver_phase,
                self.s_encoder_phase,
                self.s_target_position,
                self.s_diag_code,
                self.s_target_velocity,
            ) = self._read_drive(self.s_drive)
        except (ValueError, TypeError):
            pass
    ...
    def _set_undulator_property(
        self, movement_property, value, undulator_property
    ) -> bool:
        with self._epu_lock:
            drive_funcs = {
                ("position", "gap"): (
                    self.a_drive.set_target_position,
                    self.b_drive.set_target_position,
                ),
                ("velocity", "gap"): (
                    self.a_drive.set_target_velocity,
                    self.b_drive.set_target_velocity,
                ),
                ("position", "phase"): (
                    self.i_drive.set_target_position,
                    self.s_drive.set_target_position,
                ),
                ("velocity", "phase"): (
                    self.i_drive.set_target_velocity,
                    self.s_drive.set_target_velocity,
                ),
            }
            ...
            while retry_count > 0:
                try:
                    ...
                    if drive_a_max_velocity != drive_b_max_velocity:
                        ...
                        logger.info(
                            "Gap drives diagnostic codes do not allow movement."
                        )
                        self.message = (
                            "Gap drives diagnostic codes do not allow movement."
                        )
    ...
    def allowed_to_change_gap(self) -> bool:
        if (
            ...
            and self.gap_halt_status()
            and self._gap_check_for_move()
        ):
            ...
            while retry_count > 0:
                try:
                    ...
                    if drive_i_max_velocity != drive_s_max_velocity:
                        logger.warning(
                            "Phase drives have different maximum velocities."
                        )
                        ...
                        logger.info(
                            "Phase drives diagnostic codes do not allow movement."
                        )
                        self.message = (
                            "Phase drives diagnostic codes do not allow movement."
                        )
    ...
    def allowed_to_change_phase(self) -> bool:
        if (
            ...
            and self.phase_halt_status()
            and self._phase_check_for_move()
        ):
    ...
    def gap_set_enable(self, val: bool):
        ...
        with self._epu_lock:
            if not val and self.gap_halt_status():
                ...
                bsmp_enable_message = utils.bsmp_send(
                    _cte.BSMP_WRITE, variableID=_cte.ENABLE_CH_AB, value=val
                ).encode()
    ...
    def gap_set_halt(self, val: bool):
        ...
        with self._epu_lock:
            if val and not self.gap_enable_status():
                ...
                bsmp_enable_message = utils.bsmp_send(
                    _cte.BSMP_WRITE, variableID=_cte.HALT_CH_AB, value=val
                ).encode()
    ...
    def gap_start(self, val: bool) -> bool:
        ...
        if allow_move:
            ...
            bsmp_enable_message = utils.bsmp_send(
                _cte.BSMP_WRITE, variableID=_cte.START_CH_AB, value=val
            ).encode()
    ...
    def gap_enable_and_release_halt(self, val: bool = True) -> None:
        ...
        self.gap_enable_and_halt_released = (
            self.gap_enable_status() and not self.gap_halt_release_status()
        )
    ...
    def gap_enable_status(self) -> bool:
        with self._epu_lock:
            try:
                status = bool(
                    read_digital_status(self._gpio_socket, _cte.ENABLE_CH_AB)[-2]
                )
    ...
    def gap_halt_status(self) -> bool:
        with self._epu_lock:
            try:
                status = bool(
                    read_digital_status(self._gpio_socket, _cte.HALT_CH_AB)[-2]
                )
    ...
    def gap_turn_on(self) -> bool:
        with self._epu_lock:
            bsmp_enable_message = utils.bsmp_send(
                _cte.BSMP_WRITE, variableID=_cte.RESET_CH_AB, value=1
            ).encode()
            response = send_bsmp_message(bsmp_enable_message, self._gpio_socket)
    ...
    def phase_set_enable(self, val: bool):
        ...
        with self._epu_lock:
            if not val and self.phase_halt_status():
                ...
                bsmp_enable_message = utils.bsmp_send(
                    _cte.BSMP_WRITE, variableID=_cte.ENABLE_CH_SI, value=val
                ).encode()
    ...
    def phase_set_halt(self, val: bool):
        ...
        with self._epu_lock:
            if val and not self.phase_enable_status():
                ...
                bsmp_enable_message = utils.bsmp_send(
                    _cte.BSMP_WRITE, variableID=_cte.HALT_CH_SI, value=val
                ).encode()
    ...
    def phase_start(self, val: bool) -> bool:
        ...
        with self._epu_lock:
            if self._phase_check_for_move():
                ...
                bsmp_enable_message = utils.bsmp_send(
                    _cte.BSMP_WRITE, variableID=_cte.START_CH_SI, value=val
                ).encode()

                self.phase_start_event.set()
                response = send_bsmp_message(bsmp_enable_message, self._gpio_socket)
                logger.debug(
                    "IO server response to phase start request: {}".format(response)
                )
                return bool(response)
    ...
    def phase_enable_status(self):
        with self._epu_lock:
            try:
                status = bool(
                    read_digital_status(self._gpio_socket, _cte.ENABLE_CH_SI)[-2]
                )
    ...
    def phase_halt_status(self):
        with self._epu_lock:
            try:
                status = bool(
                    read_digital_status(self._gpio_socket, _cte.HALT_CH_SI)[-2]
                )
    ...
    def phase_turn_on(self) -> bool:
        with self._epu_lock:
            bsmp_enable_message = utils.bsmp_send(
                _cte.BSMP_WRITE, variableID=_cte.RESET_CH_SI, value=1
            ).encode()
            response = send_bsmp_message(bsmp_enable_message, self._gpio_socket)
            return True if response else False
